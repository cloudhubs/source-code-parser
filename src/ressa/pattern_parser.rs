...
impl NodePatternParser for MethodParamComponent {
    fn parse(
        ...
        // Verify
        if pattern
            .match_regexes(&self.parameter_name, &self.r#type, &mut ctx.parser)
            .is_none()
        {
            quit!(pattern.essential);
        }
...
impl NodePatternParser for FieldComponent {
    fn parse(
        ...
        // Verify
        if pattern
            .match_regexes(&*self.field_name, &self.r#type, &mut ctx.parser)
            .is_none()
        {
            quit!(pattern.essential);
        }
...
impl NodePatternParser for AnnotationComponent {
    fn parse(
        ...
        // Verify
        if pattern
            .match_regexes(&*self.name, &*self.value, &mut ctx.parser)
            .is_none()
        {
            quit!(pattern.essential);
        }
...
impl NodePatternParser for AnnotationValuePair {
    fn parse(
        ...
        if pattern
            .match_regexes(&self.key, &self.value, &mut ctx.parser)
            .is_none()
        {
            quit!(pattern.essential);
        }
